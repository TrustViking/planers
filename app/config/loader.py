"""config\\planer.yaml → PlanerConfig / ChannelConfig с проверкой каждого ключа (ТЗ §5.2)."""
...
CHANNEL_ID_PATTERN: Final[re.Pattern[str]] = re.compile(r"^[a-z][a-z0-9_]*$")
DEFAULT_MIN_LEAD_MINUTES: Final[int] = 60
DEFAULT_INBOX_KEEP_DAYS: Final[int] = 14
MIN_LEAD_MINUTES_MINIMUM: Final[int] = 0
INBOX_KEEP_DAYS_MINIMUM: Final[int] = 1
...
TOP_LEVEL_KEYS: Final[frozenset[str]] = frozenset(
    {"owner", "min_lead_minutes", "inbox_keep_days", "channels"}
)
...
def load_planer_config(path: Path) -> PlanerConfig:
    return _ConfigParser(path).parse(_read_yaml(path))


def ensure_config_exists(paths: PlanerPaths) -> bool:
    """True — planer.yaml есть; False — только что скопирован из примера (запуск надо повторить)."""
    if paths.config_file.exists():
        return True
    if not paths.config_example.exists():
        raise ConfigError(
            config_path=paths.config_file,
            key_path=msg.CONFIG_ROOT_KEY,
            problem=msg.CONFIG_PROBLEM_EXAMPLE_MISSING.format(example=paths.config_example),
        )
...
class _ConfigParser:
    ...
    def parse(self, raw: Any) -> PlanerConfig:
        ...
        return PlanerConfig(
            ...
            inbox_keep_days=self._int(
                root,
                "inbox_keep_days",
                prefix="",
                default=DEFAULT_INBOX_KEEP_DAYS,
                minimum=INBOX_KEEP_DAYS_MINIMUM,
            ),
            channels=self._channels(root),
        )
    ...
    def _channels(self, root: dict[str, Any]) -> tuple[ChannelConfig, ...]:
        raw: Any = self._required(root, "channels", prefix="")
        if not isinstance(raw, list) or not raw:
            raise self._error("channels", msg.CONFIG_PROBLEM_CHANNELS_EMPTY)
        channels: list[ChannelConfig] = []
        seen_ids: set[str] = set()
        for index, raw_channel in enumerate(raw):
            channel: ChannelConfig = self._channel(raw_channel, prefix=f"channels[{index}].")
