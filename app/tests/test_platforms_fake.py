...
def test_create_broadcast_is_deterministic_and_marked(
    ...
    created = fake_platform.create_broadcast(channel, _spec(fake_platform, slot), b"x")
    ...
    assert fake_platform.created == [FakeCall(channel.id, created.broadcast_id, slot.slot_id, b"x")]
...
def test_update_changes_texts_and_unknown_broadcast_fails(
    ...
    fake_platform.update_broadcast(channel, seeded.broadcast_id, _spec(fake_platform, slot), None)
    [listed] = fake_platform.list_upcoming(channel)
    ...
    with pytest.raises(PlatformError):
        fake_platform.update_broadcast(channel, "nope", _spec(fake_platform, slot), None)
...
def test_failures_are_configurable(
    ...
    with pytest.raises(PlatformError, match="liveStreamingNotEnabled"):
        fake_platform.create_broadcast(channel, _spec(fake_platform, slot), None)
